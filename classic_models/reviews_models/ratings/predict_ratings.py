...
def build_features(df: pd.DataFrame, preprocessing: str | Path | dict[str, Any]):
    ...
    cat_columns = preprocessing.get("cat_cols", CAT_COLUMNS)
    _validate_columns(df, text_column, cat_columns)
    ...
    cat_features = preprocessing["encoder"].transform(df[cat_columns].fillna("").astype(str))
    sparse_features = hstack([word_features, char_features, cat_features])
...
def predict_ratings(
    ...
    result = df.copy()
    result[PREDICTION_COLUMN] = np.clip(np.rint(_flatten_predictions(predictions)).astype(int), 1, 5)
    return result


def _flatten_predictions(predictions: Any) -> np.ndarray:
    if hasattr(predictions, "flatten"):
        predictions = predictions.flatten()
    return np.asarray(predictions)


def _validate_columns(df: pd.DataFrame, text_column: str, cat_columns: list[str]) -> None:
    missing = [column for column in [text_column, *cat_columns] if column not in df.columns]
    if missing:
        raise ValueError(f"DataFrame is missing columns: {missing}")


def _validate_preprocessing(preprocessing: dict[str, Any]) -> None:
    missing = {"word_vectorizer", "char_vectorizer", "encoder"} - set(preprocessing)
    if missing:
