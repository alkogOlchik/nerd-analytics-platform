...
AGENT_BEHAVIOR_HINT = (
    ...
    "\n"
    "ELEMENT TARGETING (critical for the recorder):\n"
    "* For ANY action on an interactive element (click_element, input_text, etc.)\n"
    "  ALWAYS reference the element by its numeric `index` from the highlighted list of\n"
    "  interactive elements provided in the page snapshot. Never invent xpath, CSS\n"
    "  selectors or pixel coordinates — the recorder uses `index` to draw the pointer.\n"
    "* If the desired element is NOT highlighted/indexed, first scroll into view or\n"
    "  open a parent menu so the element gets an index in the next snapshot; only then\n"
    "  click. Do not click using fabricated indices.\n"
    "* One action per step (click OR type OR scroll), so the recorder can label each\n"
    "  screenshot with exactly one target.\n"
    "\n"
